...
对于页面加载、邮箱状态或验证码提交等运行时故障，单个客户最多执行 3 次完整流程（首次尝试加 2 次重试）。每次重试都会重新创建浏览器上下文，避免上一次的页面状态影响下一次。第 1 或第 2 次成功时不会进入失败表；连续 3 次仍失败时，才会写入 `failed.xlsx`，并在“失败原因”中保留失败步骤和中文原因。输入缺失、邮箱入口无法匹配等确定性问题不会重复重试。
...
- `failed\failed.xlsx`：只包含连续 3 次尝试仍失败或需要人工处理的记录；每条都带有“状态说明”“尝试次数”“失败步骤”“失败原因”“是否可重试”，方便人工处理或单独重跑。管理员已存在的记录不会进入失败表。
- `state\run_summary.json`：本次运行的实时汇总和最后处理位置。

结果表和原始客户表格会把结果列紧接在真实输入列之后，通常就是紧跟“邮箱密码”右侧，顺序如下：

`失败原因`、`状态`、`状态说明`、`尝试次数`、`管理员操作`、`管理员结果`、`发票邮箱操作`、`发票邮箱结果`、`失败步骤`、`错误详情`、`是否可重试`、`更新时间`、`运行编号`、`原始行号`。

程序会过滤原表中没有表头的空白格式列，并会把旧版本已经写到很右侧的英文结果列迁移到这个紧凑区域。程序不会另存原表备份；写入时只使用同目录临时文件完成替换，避免中途写入半个文件。
...
- `failed / 失败`：该客户在某个步骤失败，需要查看“失败步骤”和“失败原因”。
...
程序会重新创建页面并最多重试 3 次；只有 3 次都失败才会将该客户写入失败表。请在 `failed.xlsx` 的“失败步骤”和“失败原因”中查看具体卡点。
